...
try{
   ...
   where SSN='${SSN}'`
    ...
    from (select sh_date, close from stock_history where isin = '${ISIN}' order by sh_date) sh,
    ...
    select ch.sh_date, 
        ...
        FROM (select sh.*, Decode(rownum, 1, 0, close - LAG(close, 1, 0 ) OVER (ORDER BY sh_date))  as change
            FROM (select sh_date, close from stock_history where isin = '${ISIN}' order by sh_date) sh) ch,
    ...
    select pch.*, 
        ...
        DECODE(rownum,1, 0, ROUND(change/LAG(close, 1, 0 ) OVER (ORDER BY sh_date)*100, 3)) as percent_change
        FROM (select sh.*, Decode(rownum, 1, 0, close - LAG(close, 1, 0 ) OVER (ORDER BY sh_date))  as change
            FROM (select sh_date, close from stock_history where isin = '${ISIN}' order by sh_date) sh) ch) pch
...
app.post('/MACD',(req, res) => {conn(4,req, res)});